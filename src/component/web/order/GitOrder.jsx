...
export default function GitOrder() {

  let {userToken}=useContext(UserContext);
  console.log(userToken);
const{getCartContext,removeItemContext,count,cart,loading,clearCartContext,increaseContext,decreaseContext}=useContext(CartContext);
if(loading){
  return <p>...loading</p>
}
...
const {couponName,address,phone}=values;

const {data}=await axios.post(`${import.meta.env.VITE_API_URL}/order`,{couponName,address,phone},{headers:{ authorization: `Tariq__${userToken}` }});
...
{data?.count!=0 ?data?.products.map((product)=>
    ...
    <div className="item" key={product.details._id}>
                ...
                <div className="quantity">
                  ...
                  <button  onClick={()=>IncreasGuantity(product.details._id)}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={17}
          ...
          <div className="row">
            <h2>Address</h2>
            <p>Add your address To delevere order to it</p>
            <div className="coupon-form">
            
              <input type="text" placeholder="Address" name='address'  value={formik.values.address} onChange={formik.handleChange} />
             
            </div>
          </div>
          <div className="row">
            <h2>Phone</h2>
            <p>Add your Phone number To call you </p>
            <div className="coupon-form">
            
              <input type="tel" placeholder="Phone number"  name='phone'  value={formik.values.phone} onChange={formik.handleChange}/>
          ...
          <div className="row">
            ...
            <div className="coupon-form">
              ...
              <input type="text" placeholder="Coupon Code" name='couponName'  value={formik.values.couponName} onChange={formik.handleChange}/>
              <button>Apply</button>
            </div>
          </div>
<button  type='submit' className='coupon-form mt-3  btn btn-info'>Order Now </button>
          </form>
        </div>
      </div>
    </div>
